# Paquete: Bernoulli

## Supuestos

- Audiencia adulta principiante; español mexicano cálido.
- Historia aprobada: `docs/stories/LEVEL_3.md`; escena `L3-S05`.
- Aprender y Ejercitar usan datos sintéticos narrativos; En vivo usa `Bike Sharing Dataset · UCI`.
- El narrador se renderiza como subtítulo y Don Juan conserva la decisión del negocio.

## ConceptSpec

- **Nivel/bloque:** 3, Variables aleatorias.
- **Objetivo:** Modelar un resultado binario con probabilidad de éxito.
- **Definición:** Una variable Bernoulli vale 1 si ocurre el éxito y 0 si no ocurre.
- **Intuición:** Es convertir cada día en una luz encendida o apagada.
- **Error plausible:** Olvidar que Bernoulli describe un solo ensayo binario.
- **Mecanismo visual:** codificación binaria ensayo por ensayo.
- **Estados:** Antes de comparar → Evidencia revelada.
- **Unidad:** una observación es una noche del puesto.
- **Variables:** pedidos_totales, tacos_vendidos, pedidos_para_llevar y encargo_programado.
- **Límite:** La evidencia es sintética, observacional y no identifica causalidad ni garantiza demanda futura.

## LearningModule

1. Cada noche queda como cumplió o no cumplió el umbral.
2. Ejecutar **Cambiar umbral**.
3. Cita una marca visible, responde la pregunta de Don Juan y nombra un límite.

## PracticeExercise

- **Guiado:** Incidente guiado de Bernoulli: ¿Qué representa un 1 en la variable Bernoulli de alta demanda?
- **Transferencia:** Incidente de transferencia de Bernoulli: Al subir el umbral de éxito, ¿qué debe ocurrir con la proporción de unos?
- **Bloqueo:** 1 cambios y todas las marcas requeridas.
- **Separación:** Aprender revela el mecanismo; los dos ejercicios usan incidentes y evidencia nuevos.

## LiveTeachingPack

- **Visibilidad:** `teacher-only-static`.
- **Fuente/licencia:** https://archive.ics.uci.edu/dataset/275/bike+sharing+dataset · CC BY 4.0.
- **Fecha/hash:** 2026-06-14 · `537e98e2c8b8f53e3094d953f847788b1dc224764a4a1e538b3e1ec4e30dac8a`.
- **Plan offline:** Usar HTML, snapshot y tarjetas impresas; no requiere red ni IA.

## Prompts

- **Codex:** Verifica una demo reproducible de Bernoulli con el snapshot público, cálculo y aserciones; no inventes filas ni causalidad.
- **Gemini:** Facilita preguntas socráticas sobre Bernoulli; exige predicción, evidencia y límite.
- **ChatGPT:** Revisa precisión y pedagogía de Bernoulli; detecta conclusiones que excedan el diseño.
