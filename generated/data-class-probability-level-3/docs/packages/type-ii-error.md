...
- **Concepto anterior:** Error tipo I.
- **Concepto siguiente:** Potencia.
...
3. Ejecutar **Mostrar falso negativo** y describir el cambio visible.
...
**Evidencia narrativa común:** Ejecutar «Mostrar falso negativo» y citar el cambio visible asociado con error tipo ii.
...
**Pregunta:** ¿Qué describe un error tipo II?

| Opción | Correcta | Feedback |
| --- | --- | --- |
| No rechazar H0 cuando hay un efecto real. | Sí | Tipo II es un falso negativo. |
| Rechazar H0 cuando H0 es verdadera. | No | Eso describe error tipo I. |
| Calcular mal la media muestral. | No | Un error aritmético no es el concepto estadístico. |

**Pista:** Mira el área de H1 que queda fuera del rechazo.
...
**Pregunta:** ¿Qué significa 'no significativo' con baja potencia?

| Opción | Correcta | Feedback |
| --- | --- | --- |
| Puede ser falta de sensibilidad, no prueba de ausencia de efecto. | Sí | Baja potencia aumenta falsos negativos. |
| Demuestra que el efecto es cero. | No | No rechazar no prueba igualdad exacta. |
| Elimina la necesidad de revisar tamaño de muestra. | No | La potencia depende de n, ruido y tamaño de efecto. |

**Pista:** Revisa cuánto se traslapan H0 y H1.
...
**Objetivo docente:** Reconocer el riesgo de falso negativo al no rechazar H0 cuando H1 es cierta.
...
**Evaluación rápida:** El estudiante interpreta error tipo ii citando evidencia visible, método y limitación.

**Blueprint de demo:** HTML local con snapshot real, botón «Mostrar falso negativo», dos estados visuales y aserción de cambio.
...
> Trabaja como programador docente. Usa el snapshot público indicado y verifica una demo local para «Error tipo II». Objetivo: Reconocer el riesgo de falso negativo al no rechazar H0 cuando H1 es cierta. Conserva fuente/licencia/SHA-256 visibles, no inventes filas, etiqueta cualquier simulación como didáctica derivada del snapshot y agrega una comprobación automática del cálculo.

**Gemini**

> Facilita una discusión socrática sobre «Error tipo II». Pide predicción, denominador o hipótesis antes de explicar, cuestiona interpretaciones causales y exige que cada respuesta cite la evidencia visible.

**ChatGPT**

> Actúa como revisor técnico-pedagógico de una clase sobre «Error tipo II». Detecta conclusiones que excedan el diseño, revisa si el feedback corrige errores plausibles y propone dos preguntas de transferencia con respuesta esperada.
