#!/usr/bin/env python3
"""Generate Level 13: responsible operation, monitoring and retirement."""
...
def monitoring_dataset() -> list[dict[str, object]]:
    ...
    for i in range(96):
        ...
        rows.append({
            "snapshot_id": f"L13-M{i+1:03d}", "fecha": (start + timedelta(days=i)).isoformat(), "fase": phase,
...
def config() -> dict[str, object]:
    ...
    specs = [
        ("operational-readiness","Readiness operativo","Decidir si un sistema de IA trazable puede entrar en operación bajo límites explícitos.","Readiness operativo contrasta sistema, baseline, privacidad, autoridad y reversibilidad antes de activar.","gate operativo sobre un sistema trazable existente","El equipo recibe `sistema_ia_trazable@L12.H1` y revisa si puede encenderlo sin modificar su diseño.","Si falta una condición, sigue apagado.","El gate operativo queda versionado y requiere evidencia actual.",(3,2),"L13-E1","readiness@L13.1","aprobacion_humana, procedimiento_activo"),
        ("baseline","Baseline","Comparar complejidad contra una referencia simple y relevante.","Un baseline establece referencia explícita de desempeño y costo.","referencia simple frente a propuesta","Una regla estable compite con el procedimiento nuevo.","Lo complicado tiene que ganarse su lugar.","Comparo error, estabilidad y costo contra la referencia.",(.72,.79),"L13-E1","readiness@L13.1","mae_confirmado, version_regla"),
        ("rollback","Rollback","Diseñar condición, estado seguro y verificación antes del incidente.","Rollback restaura un procedimiento anterior verificable.","señal, estado seguro y comprobación","El equipo ensaya volver a la regla r2.","Quiero saber cómo regresamos antes de avanzar.","Documento señal, dueño, pasos y verificación.",(3,4),"L13-E1","readiness@L13.1","version_regla, procedimiento_activo"),
        ("human-approval","Aprobación humana","Dar a una persona informada autoridad real para detener.","Aprobación humana requiere rol, información y poder real de detener.","bucle con evidencia, autoridad y motivo","Don Juan recibe evidencia y conserva el interruptor.","Si no puedo detenerlo, mi firma es adorno.","La aprobación deja actor, evidencia y motivo.",(1,1),"L13-E1","readiness@L13.1","aprobacion_humana, procedimiento_activo"),
        ("data-drift","Data drift","Comparar entradas actuales con una referencia versionada.","Data drift es cambio en entradas respecto de referencia.","distribuciones de referencia y periodo actual","Los pedidos suben después de la apertura del local.","Que cambie la fila no prueba que la regla ya falle.","Separo cambio de entrada de desempeño confirmado.",(78,86),"L13-E2","monitoring@L13.2","fase, pedidos_totales"),
        ("performance-drift","Performance drift","Seguir deterioro con resultados confirmados y retraso explícito.","Performance drift es deterioro de resultados medidos.","error confirmado a través del tiempo","El MAE aumenta, pero las últimas etiquetas tardan siete días.","No rellenes lo que todavía no sabemos.","Marco el retraso y no uso proxy como verdad.",(5.3,8.5),"L13-E2","monitoring@L13.2","fecha, mae_confirmado, retraso_etiqueta_dias"),
        ("calibration-drift","Calibration drift","Revisar si scores conservan correspondencia con frecuencias.","Calibration drift rompe correspondencia entre score y frecuencia.","score frente a frecuencia por periodo","Los scores suben mientras la frecuencia observada baja.","Que ordene bien no significa que diga bien la probabilidad.","Comparo curva actual con referencia y límites.",(.03,.15),"L13-E2","monitoring@L13.2","score_medio, frecuencia_observada, brecha_calibracion"),
        ("alert-threshold","Umbral de alerta","Combinar banda, persistencia, dueño y acción.","Una alerta combina umbral, duración y acción.","banda, persistencia y escalamiento","Variaciones aisladas generan ruido hasta exigir tres cortes.","Una campana que nunca calla deja de avisar.","La alerta exige persistencia y revisión humana.",(1,3),"L13-E2","monitoring@L13.2","brecha_calibracion, alerta_persistente"),
        ("triage","Triage","Priorizar por daño, alcance y reversibilidad.","Triage prioriza incidentes por daño, alcance y reversibilidad.","severidad, alcance y reversibilidad","Ocho incidentes compiten por atención.","Primero lo que puede dañar, no lo que haga más ruido.","Clasifico con criterios y escalo privacidad y equidad.",(8,3),"L13-E3","incidents@L13.3","severidad, tipo, alcance"),
        ("impact","Impacto","Registrar afectados, consecuencia, magnitud y duración.","Impacto registra quién, qué, cuánto y durante cuánto tiempo.","mapa de afectados y consecuencias","El error global oculta solicitudes de apoyo afectadas.","El promedio no recibe el daño; la gente sí.","Mapeo alcance y duración sin datos personales.",(1,4),"L13-E3","incidents@L13.3","alcance, duracion, tipo"),
        ("operational-rollback","Rollback operativo","Ejecutar reversión con responsables y comprobación.","Rollback operativo sigue pasos, responsables y verificación.","línea de tiempo de reversión","El incidente INC-04 obliga a volver a r2.","Regresar no basta: comprueba que de verdad regresó.","Detengo, revierto, valido y registro.",(3,4),"L13-E3","incidents@L13.3","incidente_id, accion, comprobacion"),
        ("postmortem","Postmortem","Reconstruir hechos y mejorar controles sin buscar culpable.","Un postmortem reconstruye hechos, controles y acciones.","ciclo de hechos, controles y acciones","El equipo revisa por qué la alerta temprana no escaló.","Arreglemos el sistema, no fabriquemos un culpable.","La acción correctiva tiene dueño y verificación.",(0,4),"L13-E3","incidents@L13.3","culpa_individual, revision_humana, comprobacion"),
        ("model-card","Model card","Documentar propósito, datos, métricas, usos y límites.","Una model card documenta propósito, datos, métricas y límites.","tarjeta de propósito y límites","Paco resume el procedimiento para quien lo reciba.","La ficha debe decir cuándo no usarlo.","Documento no reemplaza seguridad ni aprobación.",(3,4),"L13-E4","handoff@L13.4","version_datos, version_regla"),
        ("runbook","Runbook","Convertir señales en pasos verificables y escalamiento.","Un runbook convierte una condición en pasos verificables.","flujo de señal, acción, detención y ayuda","Nora ensaya la alerta sin depender de Paco.","Si el manual no dice cuándo parar, no está listo.","Cada paso tiene dueño, evidencia y salida segura.",(3,4),"L13-E4","handoff@L13.4","alerta_persistente, procedimiento_activo"),
        ("audit-log","Audit log","Registrar evento, momento, actor, versión y motivo con integridad.","Un audit log registra qué ocurrió, cuándo, quién y por qué.","cadena reconstruible de cambios","El equipo reconstruye la reversión sin sobrescribir entradas.","Quiero el camino, no solo el número final.","Cada cambio conserva versión y motivo.",(4,5),"L13-E4","handoff@L13.4","snapshot_id, fecha, version_datos, version_regla"),
        ("retirement","Retiro","Desactivar uso preservando evidencia, obligaciones y comunicación.","Retirar elimina uso activo preservando evidencia y obligaciones.","desactivar, archivar, comunicar y verificar","El procedimiento deja de aportar y el equipo practica apagarlo.","También termina bien lo que se sabe apagar.","El uso queda desactivado; archivo y dependencias se verifican.",(1,0),"L13-E4","handoff@L13.4","procedimiento_activo, version_regla"),
    ]
    items=[]
    for i,s in enumerate(specs,1):
        ...
        items.append(lesson(level=13,slug=slug,title=title,objective=objective,definition=definition,mechanism=mechanism,
    ...
    return {"level":13,"output":"data-class-operations-level-13","title":"Operación y monitoreo responsable",
        "summary":"El equipo opera un sistema de IA trazable ya diseñado: readiness, monitoreo, incidentes, rollback, auditoría y retiro.",
        "blocks":blocks,"previousConcept":"Sistema de IA trazable con harness y loop verificable","nextConcept":"Cierre de la ruta responsable",
        ...
        "narrativeDatasets":[
            {"path":"datasets/narrative/monitoreo_operativo_nivel_13.csv","rows":monitoring,"schema":monitoring_schema},
            {"path":"datasets/narrative/incidentes_operativos_nivel_13.csv","rows":incidents,"schema":incident_schema}],
        "narrativeMetadata":{"metadataPath":"datasets/narrative/nivel_13.metadata.json","id":"operacion-responsable-nivel-13","synthetic":True,
            "generator":"level13-operations-v1","seed":SEED,"period":{"start":monitoring[0]["fecha"],"end":monitoring[-1]["fecha"],"snapshots":96},
            ...
            "data_state":["sistema_ia_trazable@L12.H1","readiness@L13.1","monitoring@L13.2","incidents@L13.3","handoff@L13.4"],
